...
function SingleReel({ post, isActive }) {
  ...
  return (
    <div className="relative w-full h-[calc(100vh-64px)] max-w-sm mx-auto snap-start bg-black flex flex-col justify-center overflow-hidden rounded-3xl border border-base-content/10 shadow-2xl">
      {/* Video */}
      <div className="relative w-full h-full flex items-center justify-center bg-black">
        {post.video?.url ? (
          ...
          <div className="flex flex-col items-center justify-center p-6 text-center text-base-content/50 z-0">
        ...
        <button
          onClick={toggleMute}
          className="absolute top-4 right-4 z-30 p-2.5 rounded-full bg-black/60 text-white hover:bg-black/80 backdrop-blur transition-all pointer-events-auto"
        >
...
export default function Reels() {
      ...
      .then((res) => {
        if (res.data.success) {
          const videoPosts = (res.data.posts || []).filter(
            (p) => p.mediaType !== 'image' && p.video?.url && !p.video.url.match(/\.(jpg|jpeg|png|webp|gif)($|\?)/i)
          )
          setReels(videoPosts)
        }
      })
  ...
  if (reels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center space-y-4">
        <p className="font-display text-lg text-base-content font-bold">No Reels Uploaded Yet</p>
        <p className="text-xs text-base-content/50 max-w-xs">
          Be the first reporter to upload a video dispatch reel!
        </p>
        <Link to="/upload" className="btn btn-sm bg-primary text-white border-0 rounded-xl px-5">
          Upload Reel
        </Link>
      </div>
    )
  ...
  return (
    <div className="h-[calc(100vh-64px)] overflow-y-auto snap-y snap-mandatory py-2 space-y-6 scrollbar-none">
      {reels.map((reel, index) => (
        <SingleReel key={reel._id} post={reel} isActive={index === activeIndex} />
      ))}
    </div>
  )
}
