...
export default function Followers() {
  const { user, followersMap, registeredUsers } = useAuthStore()
  const followerIds = followersMap[user?.id] || []
  const followers = registeredUsers.filter((u) => followerIds.includes(u.id))
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-[24px] border border-white/10 bg-base-200/70 px-4 py-3 sm:px-5">
        ...
        <div>
          ...
          <p className="text-xs text-accent font-mono">{followerIds.length} people follow you</p>
        </div>
      </div>
      ...
      {followers.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 rounded-[24px] border border-dashed border-white/10 bg-base-200/40 py-16 px-6 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-accent">When someone follows you, they'll appear here.</p>
          </div>
        </div>
      ) : (
